...
const App = {
    ...
    performCalculation() {
        ...
        for (let i = 0; i < allMonths.length; i += 3) {
            ...
            const qResult = PVCCalculator.calculate({
                ...
                FB: contract.FB || 0,
                roundOff,
                indexDecimals: App.lastCalcResult && App.lastCalcResult.indexDecimals !== undefined ? App.lastCalcResult.indexDecimals : 2,
        ...
        const calcData = {
            ...
            indexDecimals: quarters[0].result.indexDecimals !== undefined ? quarters[0].result.indexDecimals : 2
        };
    ...
    renderResult(data) {
        ...
        return `
            ...
            <div class="result-actions no-print" style="display: flex; flex-wrap: wrap; align-items: center; gap: 10px; margin-bottom: 20px;">
                <div class="index-rounding-toggle" style="display: flex; align-items: center; gap: 8px; background: var(--bg-tertiary); padding: 6px 14px; border-radius: var(--radius-sm); border: 1px solid var(--border-glass); margin-right: auto;">
                    <span style="font-size:0.85rem; font-weight:600; color: var(--text-primary);">CPI/WPI Avg Rounding:</span>
                    <button class="btn btn-sm ${data.indexDecimals === 1 ? 'btn-primary' : 'btn-outline'}" onclick="App.toggleIndexDecimals(1)">1 Digit</button>
                    <button class="btn btn-sm ${data.indexDecimals !== 1 ? 'btn-primary' : 'btn-outline'}" onclick="App.toggleIndexDecimals(2)">2 Digits</button>
                </div>
    ...
    toggleIndexDecimals(dec) {
        if (!this.lastCalcResult) return;
        this.lastCalcResult.indexDecimals = dec;
        
        // Recalculate all quarters with new indexDecimals
        const contract = this.lastCalcResult.contract;
        const roundOff = this.lastCalcResult.result.roundOff;
        
        this.lastCalcResult.quarters.forEach(q => {
            q.result = PVCCalculator.calculate({
                months: q.months,
                labourPct: contract.labourPct,
                materialPct: contract.materialPct,
                fuelPct: contract.fuelPct,
                ...
                roundOff,
                indexDecimals: dec,
                indexData: q.indexData
            });
        });
        this.lastCalcResult.result = this.lastCalcResult.quarters[this.lastCalcResult.currentQuarterIndex].result;
        
        this.navigateTo('result', this.lastCalcResult);
        PVCUtils.showToast(`Switched CPI/WPI Average rounding to ${dec} decimal place${dec === 1 ? '' : 's'}`, 'success');
    },
    ...
    _generatePVCDocumentInner(c, q) {
        ...
        const indexDec = r.indexDecimals !== undefined ? r.indexDecimals : 2;
        ...
        const FQ_val = r.fuelValues ? PVCUtils.roundN(r.fuelValues.reduce((a,b)=>a+b,0)/r.fuelValues.length, indexDec) : 0;
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            const idxD = q.indexData[i] || { cpiIW: '', wpi: '', fuelWPI: '' };
            const comp = componentRows[i] || { label: '', value: 0 };
            const cpiConv = r.convertedCPI[i] !== undefined ? r.convertedCPI[i].toFixed(indexDec) : '';
            const rawCpi = idxD.cpiIW !== undefined && idxD.cpiIW !== '' ? parseFloat(idxD.cpiIW).toFixed(indexDec) : '';
            const cpiDisplay = m.label ? (parseFloat(c.linkingFactor) === 1 ? cpiConv : `${rawCpi} x ${c.linkingFactor} = ${cpiConv}`) : '';
            const rawWpi = idxD.wpi !== undefined && idxD.wpi !== '' ? parseFloat(idxD.wpi).toFixed(indexDec) : '';
            const rawFuelWpi = idxD.fuelWPI !== undefined && idxD.fuelWPI !== '' ? parseFloat(idxD.fuelWPI).toFixed(indexDec) : (idxD.wpi !== undefined && idxD.wpi !== '' ? rawWpi : '');
            const amtDisplay = m.amount !== undefined && m.amount !== '' ? parseFloat(m.amount).toFixed(dec) : '';
            
            tableRowsHTML.push(`
                <tr>
                    <td>${m.label}</td>
                    <td class="text-right">${amtDisplay ? '₹' + amtDisplay : ''}</td>
        ...
        // Summary Bill Calculations for Page 2
        const gstAmount = r.totalPVC * 0.18;
        const itAmount = r.totalPVC * 0.02;
        const tdsAmount = r.totalPVC * 0.02;
        const totalDeduction = itAmount + tdsAmount;
        const netPayableExGst = r.totalPVC - totalDeduction;
        const netPayableAmount = netPayableExGst + gstAmount;
        const netPayableRounded = Math.round(netPayableAmount);
        const netPayableWords = PVCUtils.numberToWords(Math.abs(netPayableRounded));

        return `
            <div class="pvc-page-1">
                ...
                <table class="contract-info-table compact-info-table" style="width: 100%; margin-bottom: 14px; font-size: 12px; border-collapse: collapse;">
                    <tr>
                        <td class="info-label" style="padding: 6px 10px; font-weight: 600; width: 18%; border: 1px solid #ddd; background: rgba(0,0,0,0.02);">Name of work:</td>
                        <td colspan="3" style="padding: 6px 10px; border: 1px solid #ddd;"><strong>${c.workName}</strong></td>
                    </tr>
                    <tr>
                        <td class="info-label" style="padding: 6px 10px; font-weight: 600; width: 18%; border: 1px solid #ddd; background: rgba(0,0,0,0.02);">Contract agreement no.:</td>
                        <td style="padding: 6px 10px; width: 32%; border: 1px solid #ddd;">${c.contractNumber}, dated ${PVCUtils.formatDateDot(c.contractDate)}</td>
                        <td class="info-label" style="padding: 6px 10px; font-weight: 600; width: 18%; border: 1px solid #ddd; background: rgba(0,0,0,0.02);">Tender opening date:</td>
                        <td style="padding: 6px 10px; width: 32%; border: 1px solid #ddd;">${PVCUtils.formatDateDot(c.tenderDate)}</td>
                    </tr>
                    <tr>
                        <td class="info-label" style="padding: 6px 10px; font-weight: 600; width: 18%; border: 1px solid #ddd; background: rgba(0,0,0,0.02);">PVC for the period:</td>
                        <td style="padding: 6px 10px; border: 1px solid #ddd;"><strong>${periodLabel}</strong></td>
                        <td class="info-label" style="padding: 6px 10px; font-weight: 600; width: 18%; border: 1px solid #ddd; background: rgba(0,0,0,0.02);">Base period:</td>
                        <td style="padding: 6px 10px; border: 1px solid #ddd;">${basePeriodStr}</td>
                    </tr>
                </table>
                ...
                <table class="pvc-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width:120px">CPI-IW for Labour (Base ${parseFloat(c.linkingFactor) === 1 ? (c.currentCpiYear || 2016) : (c.contractCpiYear || 2001)}=100)</th>
                            <th style="width:110px">WPI for Material Component (Base 2011-12=100)</th>
                            <th style="width:110px">WPI for Fuel & Power (Base 2011-12=100)</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${tableRowsHTML.join('')}
                        <tr class="avg-row">
                            ...
                            <td class="font-bold">${r.LQ.toFixed(indexDec)} (L<sub>Q</sub>)</td>
                            <td class="font-bold">${r.MQ.toFixed(indexDec)} (M<sub>Q</sub>)</td>
                            <td class="font-bold">${FQ_val.toFixed(indexDec)} (F<sub>Q</sub>)</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="formula-section">
                    ...
                    <div class="total-section-inline" style="margin-top: 14px; padding: 10px 16px; background: rgba(0,0,0,0.03); border: 1px solid #ccc; border-radius: 6px; font-size: 13px;">
                        <div><strong>Total price variation amount for: ${periodLabel}</strong> = ${totalFormulaStr} <strong>= ₹${r.totalPVC.toFixed(dec)}</strong></div>
                        <div class="amount-words" style="margin-top: 6px; font-style: italic;">(In words: ₹${r.amountInWords})</div>
                    </div>
                </div>
            </div>

            <div class="page-break" style="page-break-after: always; break-after: page; height: 0; margin: 0; padding: 0; border: none;"></div>

            <div class="pvc-page-2-summary" style="padding-top: 35px; font-family: 'Inter', 'Segoe UI', sans-serif; color: #000;">
                <table class="summary-bill-table" style="width: 100%; border-collapse: collapse; font-size: 13px; line-height: 1.3; border: 2px solid #000;">
                    <thead>
                        <tr>
                            <th colspan="4" style="padding: 6px 10px; text-align: center; font-size: 15px; font-weight: 700; border: 1px solid #000; background: rgba(0,0,0,0.03);">
                                Price Variation as mention in clause as per IRGCC for Service Contract
                            </th>
                        </tr>
                        <tr>
                            <th colspan="3" style="padding: 5px 10px; text-align: center; font-size: 14px; font-weight: 700; border: 1px solid #000;">
                                Contract Agreement No. - ${c.contractNumber} dated:- ${PVCUtils.formatDateDot(c.contractDate)}
                            </th>
                            <th style="padding: 5px 10px; text-align: center; font-size: 14px; font-weight: 700; border: 1px solid #000; width: 22%;">
                                Quarter-${qn}
                            </th>
                        </tr>
                        <tr>
                            <th colspan="4" style="padding: 6px 10px; text-align: left; font-size: 13px; font-weight: 500; border: 1px solid #000;">
                                <strong>Name of Work:-</strong> ${c.workName}
                            </th>
                        </tr>
                        <tr style="background: rgba(0,0,0,0.03);">
                            <th style="padding: 5px 10px; text-align: center; font-weight: 700; border: 1px solid #000; width: 10%;">Sr. No.</th>
                            <th style="padding: 5px 10px; text-align: center; font-weight: 700; border: 1px solid #000; width: 38%;">Component</th>
                            <th style="padding: 5px 10px; text-align: center; font-weight: 700; border: 1px solid #000; width: 30%;">MONTH</th>
                            <th style="padding: 5px 10px; text-align: center; font-weight: 700; border: 1px solid #000; width: 22%;">PVC BILL AMOUNT</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">1</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">Labour (A)</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">${periodLabel}</td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${r.labourVariation.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">2</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">Other Material (B)</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">${periodLabel}</td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${r.materialVariation.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">3</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">Fuel (C)</td>
                            <td style="padding: 5px 10px; text-align: center; border: 1px solid #000;">${periodLabel}</td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${r.fuelVariation.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: center; font-weight: 700; font-size: 13.5px; border: 1px solid #000;">Total Amount (A+B+C)</td>
                            <td style="padding: 5px 10px; text-align: right; font-weight: 700; font-size: 13.5px; border: 1px solid #000;">₹${r.totalPVC.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: center; border: 1px solid #000;"><span style="color: #2563eb; font-weight: 600; text-decoration: underline;">GST@18%</span></td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${gstAmount.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; border: 1px solid #000;">Income Tax 2% on Total Amount</td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${itAmount.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; border: 1px solid #000;">TDS Deduction 2% on total amount</td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${tdsAmount.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; font-weight: 600; border: 1px solid #000;">Total Deduction</td>
                            <td style="padding: 5px 10px; text-align: right; font-weight: 600; border: 1px solid #000;">₹${totalDeduction.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; font-weight: 600; border: 1px solid #000;">Net Payable(Gross amount-Total Deduction)</td>
                            <td style="padding: 5px 10px; text-align: right; font-weight: 600; border: 1px solid #000;">₹${netPayableExGst.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; border: 1px solid #000;"><span style="color: #2563eb; font-weight: 600; text-decoration: underline;">GST@18%</span></td>
                            <td style="padding: 5px 10px; text-align: right; border: 1px solid #000;">₹${gstAmount.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; font-weight: 600; border: 1px solid #000;">Net Payable Amount</td>
                            <td style="padding: 5px 10px; text-align: right; font-weight: 600; border: 1px solid #000;">₹${netPayableAmount.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" style="padding: 5px 10px; text-align: left; font-weight: 700; border: 1px solid #000;">Net Payable Amount (Round off)</td>
                            <td style="padding: 5px 10px; text-align: right; font-weight: 700; border: 1px solid #000;">₹${netPayableRounded.toFixed(dec)}</td>
                        </tr>
                        <tr>
                            <td colspan="4" style="padding: 6px 10px; text-align: left; font-weight: 600; border: 1px solid #000;">
                                Net payable Amount(In word):- ${netPayableWords}
                            </td>
                        </tr>
                    </tbody>
                </table>
