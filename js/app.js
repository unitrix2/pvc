...
const App = {
    ...
    renderCalculate() {
        ...
        if (contracts.length === 0) {
            return `
                ...
                <div class="empty-state">
                    <div class="empty-icon">📋</div>
                    <p>No contracts found. Please add a contract first.</p>
                    <button class="btn btn-primary" onclick="App.navigateTo('contracts')">➕ Add Contract</button>
                </div>
            `;
        }
        ...
        return `
            ...
            <div class="card no-print">
                <div class="card-header">
                    <h3>📤 Upload Quarter Data (Auto-Fill)</h3>
                ...
                <div class="upload-zone">
                    ...
                    <p>Upload filled Quarter Data Excel to auto-fill this form</p>
                </div>
            </div>
    ...
    bindCalculateEvents() {
        ...
        const updateMonthRows = () => this.updateMonthRows();
    ...
    updateMonthRows() {
        ...
        const months = PVCUtils.getConsecutiveMonths(sm, sy, 3);

        container.innerHTML = months.map((m, i) => {
            const idx = PVCStorage.getIndexForMonth(m.year, m.month);
            return `
                <div class="month-data-grid">
                    <div class="month-label">${m.label}</div>
                    <div class="form-group">
                        <label class="form-label">Payable Amount (₹)</label>
                        <input type="number" class="form-input" id="amount-${i}" step="any" placeholder="0.00" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">CPI-IW (New Series)</label>
                        <input type="number" class="form-input" id="cpi-${i}" step="any" value="${idx ? idx.cpiIW : ''}" placeholder="e.g., 121.7" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">WPI</label>
                        <input type="number" class="form-input" id="wpi-${i}" step="any" value="${idx ? idx.wpi : ''}" placeholder="e.g., 133.7" required>
                    </div>
                </div>
            `;
        }).join('');
    ...
    performCalculation() {
        ...
        const quarterNumber = parseInt(document.getElementById('quarter-number').value);
        const sm = parseInt(document.getElementById('start-month').value);
        const sy = parseInt(document.getElementById('start-year').value);
        const monthsInfo = PVCUtils.getConsecutiveMonths(sm, sy, 3);
        ...
        for (let i = 0; i < 3; i++) {
            ...
            indexData.push({ cpiIW, wpi, fuelWPI });
        }

        // Run calculation
        const result = PVCCalculator.calculate({
            months,
            ...
            MB: contract.MB,
            FB: contract.FB || 0,
            roundOff: parseInt(document.getElementById('round-off').value) || 2,
            indexData
        });
        ...
        const calcData = {
            ...
            quarterNumber,
            periodLabel: `${months[0].label} to ${months[2].label}`,
            months,
            indexData,
            result
        };
    ...
    renderResult(data) {
        ...
        const r = data.result;
        const qn = data.quarterNumber;
        const periodLabel = data.periodLabel;

        // Generate document HTML
        const docHTML = this._generatePVCDocument(data);
        ...
        return `
            ...
            <div class="result-actions no-print">
                ...
                <button class="btn btn-secondary" onclick="PDFExport.exportToPDF('pvc-document', 'PVC_Quarter${qn}_${periodLabel.replace(/ /g,'')}.pdf')">📄 Download Image PDF</button>
                <button class="btn btn-secondary" onclick="PDFExport.printDocument()">🖨️ Print / Save as PDF (Searchable)</button>
    ...
    _generatePVCDocument(data) {
        ...
        const FQ_val = r.fuelValues ? PVCUtils.roundN(r.fuelValues.reduce((a,b)=>a+b,0)/r.fuelValues.length, dec) : 0;
        ...
        return `
            <div class="pvc-document" id="pvc-document">
                    ...
                    <tbody>
                        ${months.map((m, i) => `
                            <tr>
                                ...
                                ${i === 0 ? `<td rowspan="3" class="font-bold" style="vertical-align:middle">${r.W}</td>` : ''}
                                <td class="text-left" style="font-size:10.5px">${componentRows[i].label}</td>
                                <td class="text-right">${componentRows[i].value.toFixed(dec)}</td>
                                <td>${parseFloat(c.linkingFactor) === 1 ? r.convertedCPI[i].toFixed(dec) : `${data.indexData[i].cpiIW} x ${c.linkingFactor} = ${r.convertedCPI[i].toFixed(dec)}`}</td>
                                <td>${data.indexData[i].wpi}</td>
                            </tr>
                        `).join('')}
                        <tr class="avg-row">
                            <td colspan="3"></td>
                            <td colspan="2" style="text-align:right;font-weight:700">Average price index</td>
                            <td class="font-bold">${r.LQ.toFixed(dec)} (L<sub>Q</sub>)</td>
                            <td class="font-bold">${r.MQ.toFixed(dec)} (M<sub>Q</sub>)</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="formula-section">
                    ...
                    <div class="formula-row-compact">
                        ...
                        <span class="formula-result">= <strong>₹${r.labourVariation.toFixed(2)}</strong></span>
                    </div>
                    ...
                    <div class="formula-row-compact">
                        ...
                        <span class="formula-result">= <strong>₹${r.materialVariation.toFixed(2)}</strong></span>
                    </div>
