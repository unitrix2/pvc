...
const PVCCalculator = {
    ...
    calculate(params) {
        ...
        const indexDec = params.indexDecimals !== undefined ? params.indexDecimals : 2;
        ...
        const convertedCPI = indexData.map(d => PVCUtils.roundN(parseFloat(d.cpiIW) * linkingFactor, indexDec));

        // ===== Average Indices =====
        // LQ: Average of converted CPI-IW values (strictly rounded to indexDec decimals)
        const LQ = PVCUtils.roundN(convertedCPI.reduce((sum, v) => sum + v, 0) / convertedCPI.length, indexDec);

        // MQ: Average of WPI values (strictly rounded to indexDec decimals)
        const wpiValues = indexData.map(d => PVCUtils.roundN(parseFloat(d.wpi), indexDec));
        const MQ = PVCUtils.roundN(wpiValues.reduce((sum, v) => sum + v, 0) / wpiValues.length, indexDec);

        // FQ: Average of Fuel WPI values (strictly rounded to indexDec decimals)
        const fuelValues = indexData.map(d => PVCUtils.roundN(parseFloat(d.fuelWPI || d.wpi), indexDec));
        const FQ = PVCUtils.roundN(fuelValues.reduce((sum, v) => sum + v, 0) / fuelValues.length, indexDec);
