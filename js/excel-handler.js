...
const ExcelHandler = {
    ...
    exportBulkExcel(data) {
        ...
        data.quarters.forEach(q => {
            ...
            const indexFormat = indexDec === 1 ? '#,##0.0' : '#,##0.00';
            const qData = [
            ...
            q.months.forEach((m, idx) => {
                ...
                qData.push([
                    ...
                    idxData.cpiIW ? parseFloat(parseFloat(idxData.cpiIW).toFixed(indexDec)) : '',
                    r.convertedCPI[idx] !== undefined ? parseFloat(parseFloat(r.convertedCPI[idx]).toFixed(indexDec)) : '',
                    idxData.wpi ? parseFloat(parseFloat(idxData.wpi).toFixed(indexDec)) : '',
                    idxData.fuelWPI ? parseFloat(parseFloat(idxData.fuelWPI).toFixed(indexDec)) : ''
                ]);
            });
            ...
            for (let rRow = 0; rRow < qData.length; rRow++) {
                for (let cCol = 0; cCol < 6; cCol++) {
                    ...
                    if (rRow === 0) wsQ[cellAddr].s = styleHeader;
                        ...
                        wsQ[cellAddr].s = cCol === 0 ? styleTextData : (cCol === 1 ? { ...styleData, z: '#,##0.00' } : { ...styleData, z: indexFormat });
                    } else if (rRow === startAvg + 1 || rRow === startAvg + 2) {
                        wsQ[cellAddr].s = cCol === 0 ? styleLabel : (cCol === 1 ? { ...styleData, z: indexFormat } : {});
                    } else if (rRow >= startVar + 1 && rRow <= startVar + 3) {
